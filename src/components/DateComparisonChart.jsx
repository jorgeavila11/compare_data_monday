...
function DateComparisonChart() {
    ...
    useEffect(() => {
        const fetchContext = async () => {
            try {
                ...
                const storageRes = await monday.storage.getItem("selectedBoardId");
                if (storageRes?.data?.value) {
                    setBoardId(storageRes.data.value);
                }

                // Depois verifica o contexto atual
                const contextRes = await monday.get('context');
                if (contextRes?.data?.boardId) {
                    const id = parseInt(contextRes.data.boardId);
                    if (!isNaN(id)) {
                        setBoardId(id);
                        await monday.storage.setItem("selectedBoardId", contextRes.data.boardId);
                    } else {
                        setError("ID do board inválido");
                    }
                }
                ...
                monday.listen("context", async (res) => {
                    if (res?.data?.boardId) {
                        const id = parseInt(res.data.boardId);
                        if (!isNaN(id)) {
                            setBoardId(id);
                            await monday.storage.setItem("selectedBoardId", res.data.boardId);
                        }
                    }
                });
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const option = {
            title: {
                text: `Comparativo entre Datas - Todos os Itens`,
                subtext: `${column1Title} vs ${column2Title}`,
                left: 'center',
                textStyle: {
                    fontSize: 16,
                    fontWeight: 'bold'
